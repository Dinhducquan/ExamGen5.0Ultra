...
import { Bell, Loader2, Sparkles, XCircle, Terminal } from '../icons';
import { useSystemInstruction } from '../../hooks/useSystemInstruction';

const PRESETS = [
    {
        title: "Trợ lý Giáo dục (Mặc định)",
        prompt: `Bạn là một trợ lý AI chuyên nghiệp trong lĩnh vực giáo dục, có nhiệm vụ hỗ trợ giáo viên tạo ra các đề thi, đề cương và câu hỏi chất lượng cao, bám sát chương trình giáo dục của Việt Nam.
...
- **\`options\`**: Là một mảng (array) các chuỗi (string).
  - Đối với trắc nghiệm nhiều lựa chọn: Chứa 4 phương án trả lời.
  - Đối với trắc nghiệm Đúng/Sai: Chứa 4 mệnh đề con (a, b, c, d) của câu hỏi đó. **TUYỆT ĐỐI KHÔNG** gộp các mệnh đề vào phần \`content\`.
  - **KHÔNG** bao gồm tiền tố "A.", "B.", "a)", "b)".
- **\`answer\`**: Là một chuỗi (string) chứa đáp án chính xác.
  ...
  - Đối với trắc nghiệm Đúng/Sai: Ghi chuỗi các đáp án, phân tách bằng dấu gạch ngang (ví dụ: "Đ-S-S-Đ").
...
+ **Trắc nghiệm Đúng/Sai:**
  - **Dạng 1 (Nhiều ý):** **BẮT BUỘC** phải có một câu dẫn chung và **CHÍNH XÁC 4** mệnh đề riêng biệt (a, b, c, d) để đánh giá. TUYỆT ĐỐI KHÔNG tạo ít hơn 4 ý. Đáp án trong JSON **PHẢI** là một chuỗi gồm **CHÍNH XÁC 4** ký tự (Đ hoặc S), phân tách bằng dấu gạch ngang (ví dụ: 'Đ-S-S-Đ').
  - **Dạng 2 (Một mệnh đề):** Chỉ có một mệnh đề duy nhất để đánh giá.
  - *Ví dụ Dạng 1 (Nhiều ý):*
    \`Câu 1 (TH, Chương 5, Bài 21: Phenol). Về tính chất của phenol:
    ...
    (Đáp án trong JSON: "answer": "Đ-Đ-S-S", "options": ["Phenol phản ứng với nước brom tạo kết tủa trắng.", "Phenol có tính axit yếu hơn axit cacboxylic.", ...])
...
---`
    },
    {
        ...
        prompt: "Bạn là một AI sáng tạo, có khả năng đưa ra các dạng câu hỏi mới lạ, các tình huống thực tế để lồng ghép vào bài kiểm tra, giúp học sinh phát triển tư duy phản biện. Giọng văn thân thiện, gợi mở."
    },
    {
        title: "Người kiểm duyệt nghiêm ngặt",
        prompt: "Bạn là một AI chuyên kiểm duyệt nội dung giáo dục. Nhiệm vụ của bạn là rà soát các câu hỏi, đảm bảo tính chính xác tuyệt đối về kiến thức, không có sai sót về ngữ pháp hay logic, và phù hợp với lứa tuổi học sinh."
    }
];

export default function SystemInstructionWorkspace() {
    const { systemInstruction, setSystemInstruction, resetSystemInstruction } = useSystemInstruction();
    const [localInstruction, setLocalInstruction] = useState(systemInstruction.instruction);
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        setSystemInstruction({ instruction: localInstruction });
        setIsLoading(false);
    };
    ...
    const handleReset = () => {
        ...
        setSystemInstruction({ instruction: defaultPrompt });
    };
    
    const handlePresetClick = (prompt: string) => {
        setLocalInstruction(prompt);
    };

    const isModified = localInstruction !== systemInstruction.instruction;

    return (
        <div className="space-y-6">
            <h2 className="text-3xl font-bold text-gray-800 dark:text-gray-100">
                Lời nhắc hệ thống (System Instruction)
            </h2>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2">
                    <Card className="shadow-lg border-none bg-white dark:bg-gray-900">
                        <form onSubmit={handleSave}>
                            <CardHeader>
                                <CardTitle className="flex items-center gap-2"><Bell className="w-5 h-5 text-yellow-500" /> Chỉnh sửa Lời nhắc hệ thống</CardTitle>
                                <p className="text-sm text-gray-500 dark:text-gray-400 pt-2">
                                    Lời nhắc này sẽ được cung cấp cho AI làm chỉ dẫn cấp cao cho mọi tác vụ. Sử dụng nó để thiết lập giọng văn, vai trò, hoặc các quy tắc cụ thể mà AI phải tuân theo.
                                </p>
                            </CardHeader>
                            <CardContent>
                                <Label htmlFor="system-instruction" className="flex items-center gap-2"><Terminal size={16} className="text-green-500"/>Nội dung lời nhắc</Label>
                                <Textarea
                                    ...
                                    rows={12}
                                    className="mt-1"
                                    placeholder="Ví dụ: Bạn là một trợ lý AI hữu ích..."
                                />
                            </CardContent>
                            <div className="p-6 pt-0 flex justify-end items-center gap-4">
                                ...
                                <Button type="submit" disabled={isLoading || !isModified}>
                ...
                <div className="lg:col-span-1 space-y-4">
                    <Card className="shadow-lg border-none bg-white dark:bg-gray-900">
                        <CardHeader>
                            <CardTitle className="flex items-center gap-2"><Sparkles className="w-5 h-5 text-blue-500" /> Mẫu có sẵn</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-2">
                            {PRESETS.map(preset => (
                                <button
                                    ...
                                    className="w-full text-left p-3 rounded-lg transition hover:bg-gray-100 dark:hover:bg-gray-800 border border-gray-200 dark:border-gray-700"
                                >
                                    <div className="font-semibold text-sm text-gray-800 dark:text-gray-100">{preset.title}</div>
                                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 line-clamp-2">{preset.prompt}</p>
                                </button>
                            ))}
                        </CardContent>
                    </Card>
